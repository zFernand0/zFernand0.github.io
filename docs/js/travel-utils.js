...
class TravelUtils {
  ...
  updateFilteredView() {
    ...
    cards.forEach(card => {
      const showCity = this.currentFilters.city === "all" || card.dataset.city === this.currentFilters.city;
      const showTheme = this.currentFilters.theme === "all" || card.dataset.theme === this.currentFilters.theme;
  ...
  populateActivities(activities, containerId) {
    ...
    activities.forEach(activity => {
      ...
      const cityColor = activity.city === "tokyo" || activity.city === "osaka" ? "bg-[#C84630]" : "bg-[#3E606F]";
  ...
  createActivityChart(activities, chartId, options = {}) {
    ...
    const data = themes.map(theme => activities.filter(a => a.theme === theme).length).filter(count => count > 0);

    const labels = themes
      .filter(theme => activities.filter(a => a.theme === theme).length > 0)
